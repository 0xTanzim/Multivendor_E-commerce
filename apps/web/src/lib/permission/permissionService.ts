...
/**
 * Service for managing role-based permissions with hierarchical inheritance.
...
export const permissionService = {
  ...
  async loadAll(): Promise<RolePermissionMap> {
    ...
    // Cache the result
    await redis.set(CACHE_KEY, finalMap);
  ...
  async getPermissions(roleId: string): Promise<string[]> {
    let map = await redis.get<RolePermissionMap>(CACHE_KEY);
